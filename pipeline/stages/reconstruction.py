...
class ReconstructionFilter:
    ...
    def digital_to_analog(digital_signal, fs, cutoff_freq, order=5):
        """
    ...
    def process(self, digital_signal: np.ndarray, environment: str, 
               sample_rate: int = 44100) -> np.ndarray:
        logger.info(f"DAC reconstruction for {environment} environment")
        
        if digital_signal.ndim > 1:
            digital_signal = digital_signal[:, 0]
        ...
        return t_audio_analog, audio_analog
